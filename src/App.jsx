...
export default function App() {
  ...
  const [editProjMod, setEPM] = useState(null); const [epN, setEPN] = useState(""); const [epA, setEPA] = useState(""); const [epAct, setEPAct] = useState(true);
  ...
  const editProj = async () => { if (!epN.trim()) return; try { await api(`projects?id=eq.${editProjMod}`, { method: "PATCH", body: JSON.stringify({ name: epN.trim(), address: epA.trim(), active: epAct }) }); await load(); setEPM(null); show("Updated"); } catch (e) { show("Error"); } };
  ...
  return <div style={{ minHeight: "100vh", background: P.bg, fontFamily: F.b, paddingBottom: 80 }}><style>{css}</style>
            ...
            <div key={p.id} style={{ padding: "12px 16px", background: "#fff", borderRadius: 12, border: `1px solid ${P.bd}`, marginBottom: 8, borderLeft: `3px solid ${p.active !== false ? P.tn : P.l}`, opacity: p.active !== false ? 1 : .6 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                ...
                <div style={{ display: "flex", gap: 4 }}>
                  <button onClick={() => { setEPM(p.id); setEPN(p.name); setEPA(p.address || ""); setEPAct(p.active !== false); }} style={{ fontSize: 11, padding: "4px 10px", borderRadius: 6, border: `1px solid ${P.bd}`, background: "#fff", color: P.m, cursor: "pointer", fontFamily: F.m }}>Edit</button>
    ...
    <Modal open={!!editProjMod} onClose={() => setEPM(null)} title="Edit Project"><Fl l="Name"><input style={iS} value={epN} onChange={e => setEPN(e.target.value)} /></Fl><Fl l="Address"><input style={iS} value={epA} onChange={e => setEPA(e.target.value)} /></Fl><Fl l="Status"><div style={{ display: "flex", gap: 6 }}>{[{ v: true, l: "Active" }, { v: false, l: "Archived" }].map(o => <button key={o.l} onClick={() => setEPAct(o.v)} style={{ flex: 1, padding: 10, borderRadius: 8, border: `1.5px solid ${epAct === o.v ? (o.v ? P.g : P.am) : P.bd}`, background: epAct === o.v ? (o.v ? P.gB : P.aB) : "#fff", color: epAct === o.v ? (o.v ? P.g : P.am) : P.m, fontSize: 14, fontWeight: 600, cursor: "pointer" }}>{o.l}</button>)}</div></Fl><Btn full disabled={!epN.trim()} onClick={editProj}>Save</Btn></Modal>
