...
function AddressInput({ value, onChange, placeholder }) {
  ...
  useEffect(() => {
    ...
    autocompleteRef.current.addListener('place_changed', () => {
      ...
      if (place.formatted_address) onChange({ target: { value: place.formatted_address } });
    });
  }, [onChange]);
  
  return <input ref={inputRef} style={iS} value={value} onChange={onChange} placeholder={placeholder} />;
}
...
export default function App() {
  ...
  const [projMod, setProjMod] = useState({ o: false, proj: null }); const [nPN, setNPN] = useState(""); const [nPA, setNPA] = useState("");
  ...
  const doTxn = async (matId, qty, projId, note, md, personName) => {
    ...
    try {
      ...
      await api("transactions", { method: "POST", body: JSON.stringify({ material_id: matId, material_name: mat.name, unit: mat.unit, qty, mode: md, project_id: projId || null, project_name: proj?.name || "", note, user_name: personName, user_id: user.id }) });
  ...
  const checkOut = async (toolId, toolName, projId, note, personName) => {
    const proj = projs.find(p => p.id === projId);
    try { await api("tool_checkouts", { method: "POST", body: JSON.stringify({ tool_id: toolId, tool_name: toolName, checked_out_by: personName, user_id: user.id, project_id: projId, project_name: proj?.name || "", note }) }); await load(); setToolCoMod({ o: false, tool: null }); show(`${toolName} checked out`); } catch (e) { show("Error"); }
  };
  ...
  const saveProj = async (proj) => { 
    try { 
      if (proj.id) {
        await api(`projects?id=eq.${proj.id}`, { method: "PATCH", body: JSON.stringify({ name: proj.name, address: proj.address }) });
        show("Project updated");
      } else {
        await api("projects", { method: "POST", body: JSON.stringify({ name: proj.name, address: proj.address }) }); 
        show("Added");
      }
      await load(); setProjMod({ o: false, proj: null }); setNPN(""); setNPA(""); 
    } catch (e) { show("Error"); } 
  };
  ...
  if (detail) {
    ...
    return <div style={{ minHeight: "100vh", background: P.bg, fontFamily: F.b, paddingBottom: 80 }}><style>{css}</style>
      ...
      {txnMod.o && txnMod.mat?.id === mat.id && <TxnSheet mat={mat} mode={txnMod.m} projs={projs} userName={user.name} onClose={() => setTxnMod({ o: false, m: "take", mat: null })} onSubmit={doTxn} />}
  ...
  return <div style={{ minHeight: "100vh", background: P.bg, fontFamily: F.b, paddingBottom: 80 }}><style>{css}</style>
    ...
    <div style={{ padding: "16px 16px 0" }}>
        ...
        {tools.map(tool => { const co = cos.find(c => c.tool_id === tool.id && !c.returned_at); const isOut = !!co;
          return <div key={tool.id} style={{ background: "#fff", borderRadius: 14, padding: "14px 16px", marginBottom: 8, border: `1px solid ${isOut ? "rgba(217,119,6,.3)" : P.bd}`, borderLeft: `3px solid ${isOut ? P.am : P.tn}` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
              <div><div style={{ fontSize: 15, fontWeight: 700 }}>{tool.name}</div>{tool.serial_number && <div style={{ fontSize: 11, fontFamily: F.m, color: P.l }}>S/N: {tool.serial_number}</div>}{tool.location && <div style={{ fontSize: 11, fontFamily: F.m, color: P.m }}>📍 {tool.location}</div>}<CondBadge c={tool.condition || "Good"} /></div>
      ...
      {tab === "projects" && <div style={{ animation: "fadeIn .3s" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}><h2 style={{ fontFamily: F.h, fontSize: 20, fontWeight: 700, margin: 0 }}>Projects</h2>{isA && <Btn small onClick={() => setProjMod({ o: true, proj: null })}>+ Add</Btn>}</div>
        ...
        {projs.filter(p => pFilter === "active" ? p.active !== false : pFilter === "archived" ? p.active === false : true).map(p =>
          <div key={p.id} style={{ padding: "12px 16px", background: "#fff", borderRadius: 12, border: `1px solid ${P.bd}`, marginBottom: 8, borderLeft: `3px solid ${p.active !== false ? P.tn : P.l}`, opacity: p.active !== false ? 1 : .6 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}><div><div style={{ fontWeight: 700, fontSize: 15 }}>{p.name}</div><div style={{ fontSize: 12, color: P.l }}>{p.address}</div></div>
              {isA && <div style={{ display: "flex", gap: 4 }}><button onClick={() => setProjMod({ o: true, proj: p })} style={{ fontSize: 11, padding: "4px 10px", borderRadius: 6, border: "none", background: P.tB, color: P.bk, fontWeight: 600, cursor: "pointer", fontFamily: F.m }}>Edit</button><button onClick={() => togProj(p.id, p.active !== false)} style={{ fontSize: 11, padding: "4px 10px", borderRadius: 6, border: "none", background: p.active !== false ? P.aB : P.gB, color: p.active !== false ? P.am : P.g, fontWeight: 600, cursor: "pointer", fontFamily: F.m }}>{p.active !== false ? "Archive" : "Restore"}</button></div>}
            </div>
          </div>)}
      </div>}
    ...
    {txnMod.o && !detail && <TxnSheet mat={txnMod.mat} mode={txnMod.m} projs={projs} userName={user.name} onClose={() => setTxnMod({ o: false, m: "take", mat: null })} onSubmit={doTxn} />}
    ...
    <Modal open={projMod.o} onClose={() => setProjMod({ o: false, proj: null })} title={projMod.proj ? "Edit Project" : "Add Project"}>
      <Fl l="Name"><input style={iS} value={projMod.proj ? projMod.proj.name : nPN} onChange={e => projMod.proj ? setProjMod({...projMod, proj: {...projMod.proj, name: e.target.value}}) : setNPN(e.target.value)} placeholder="e.g. Henderson Patio" /></Fl>
      <Fl l="Address">{mapsLoaded ? <AddressInput value={projMod.proj ? projMod.proj.address : nPA} onChange={e => projMod.proj ? setProjMod({...projMod, proj: {...projMod.proj, address: e.target.value}}) : setNPA(e.target.value)} placeholder="123 Main St, Denver, CO" /> : <input style={iS} value={projMod.proj ? projMod.proj.address : nPA} onChange={e => projMod.proj ? setProjMod({...projMod, proj: {...projMod.proj, address: e.target.value}}) : setNPA(e.target.value)} placeholder="123 Main St, Denver, CO" />}</Fl>
      <Btn full disabled={projMod.proj ? !projMod.proj.name.trim() : !nPN.trim()} onClick={() => saveProj(projMod.proj || { name: nPN, address: nPA })}>{projMod.proj ? "Save Changes" : "Add Project"}</Btn>
    </Modal>
...
function TxnSheet({ mat, mode, projs, userName, onClose, onSubmit }) {
  const [q, setQ] = useState(""); const [pId, setPId] = useState(""); const [nt, setNt] = useState(""); const [pName, setPName] = useState(userName);
  if (!mat) return null;
  ...
  return <Modal open={true} onClose={onClose} title={isTake ? `Take ${mat.name}` : `Add ${mat.name}`}>
    ...
    <Fl l="Person"><input style={iS} value={pName} onChange={e => setPName(e.target.value)} placeholder={userName} /></Fl>
    ...
    <Btn full disabled={!ok || !pName.trim()} color={isTake ? P.r : P.g} onClick={() => onSubmit(mat.id, qn, pId, nt, mode, pName)}>{isTake ? `Take ${qn}` : `Add ${qn}`} {mat.unit}</Btn>
  </Modal>;
}
...
function ToolForm({ tool, onSave, onCancel, isEdit, editReason, setEditReason }) {
  const [f, setF] = useState(tool || { name: "", serial_number: "", condition: "Good", notes: "", location: "" });
  const s = (k, v) => setF(p => ({ ...p, [k]: v }));
  return <div>
    ...
    <Fl l="Location"><input style={iS} value={f.location} onChange={e => s("location", e.target.value)} placeholder="e.g. Bay A-1, Truck 3" /></Fl>
    <Fl l="Condition"><div style={{ display: "flex", gap: 6 }}>
      {CONDS.map(c => <button key={c} onClick={() => s("condition", c)} style={{ flex: 1, padding: "8px", borderRadius: 8, border: `1.5px solid ${f.condition === c ? (COND_C[c] || P.am) : P.bd}`, background: f.condition === c ? `${COND_C[c]}15` : "#fff", color: f.condition === c ? COND_C[c] : P.m, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>{c}</button>)}
    </div></Fl>
    <Fl l="Notes"><input style={iS} value={f.notes} onChange={e => s("notes", e.target.value)} /></Fl>
...
function ToolCoSheet({ tool, projs, userName, onClose, onSubmit }) {
  const [pId, setPId] = useState(""); const [nt, setNt] = useState(""); const [pName, setPName] = useState(userName);
  return <Modal open={true} onClose={onClose} title={`Check Out: ${tool.name}`}>
    ...
    <Fl l="Note"><input style={iS} value={nt} onChange={e => setNt(e.target.value)} /></Fl>
    <Btn full disabled={!pId || !pName.trim()} onClick={() => onSubmit(tool.id, tool.name, pId, nt, pName)}>Check Out</Btn>
  </Modal>;
}
